...
namespace HutongGames.PlayMaker.Actions.NoesisGUI
{
    ...
    [Tooltip("Sets the outer margin of an element")]
    public class SetMargin : FsmStateAction
    {
        [RequiredField]
        ...
        private void Update()
        {
            if (_control != null)
            {
                _control.Margin = new Thickness(left.Value, top.Value, right.Value, bottom.Value);
            }
        }
